...
default_server_test () ->
    ...
    {ok, "Click here for next page" ++ _Rest} = selenium_api:getText (Session,"link"),
    
    {ok, StringLinks} = selenium_api: getAllLinks (Session),
    true = length (StringLinks) > 3,
    "linkToAnchorOnThisPage" = lists: nth (4, StringLinks),
    
    selenium_api: click (Session, "link"),
    selenium_api: waitForPageToLoad (Session, "5000"),
...
google_test () ->
    ...
    selenium_api: waitForPageToLoad (Session, "5000"),
    {ok,"hello world - Google Search"} = selenium_api: getTitle (Session),
    selenium: stop (Session),
    ok.
...
keypress_test () ->
    ...
    selenium_api: keyPress (Session, InputId, "74"),
    receive after 500 -> ok end,
    selenium_api: keyPress (Session, InputId, "97"),
    selenium_api: keyPress (Session, InputId, "110"),
    receive after 500 -> ok end,
    {ok, "Jane Agnews"} = selenium_api: getText (Session, UpdateId),
    selenium_api: keyPress(Session, InputId, "13"),
    receive after 500 -> ok end,
    {ok, "Jane Agnews"} = selenium_api: getValue (Session, InputId),
    selenium: stop (Session).
...
type_very_long_text_test () ->
    ...
    {ok, LongText} = selenium_api: getValue (Session, "richtext"),
    selenium: stop (Session).
...
utf8_test () ->    
    ...
    Test = fun(Text) ->
		   ...
		   {ok, Text} = selenium_api: getValue (Session,Object)
	   end,
...
i18n_test () ->
    ...
    Test = fun({Id,Data}) ->
		   ...
		   Result = selenium_api: isTextPresent (Session, UTF8),
		   {ok, "true"} = Result,
		   {ok, UTF8} = selenium_api: getText (Session, Id)
	   end,
